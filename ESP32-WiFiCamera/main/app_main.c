...
static void sendJPGStream(int socket) {
    ESP_LOGE(TAG, "Starting JPG stream.");
   while(true) {
    camera_run();
    int size = camera_get_data_size();
    char *data = (char *)camera_get_fb();
    ESP_LOGE(TAG, "siz = %d", size);
    int err;
    err = send(socket, &size, 4, 0);
    if(err < 0) break;
    err = send(socket, data, size, 0);
    if(err < 0) break;
   }
   ESP_LOGE(TAG, "Stream ends.");
   close(socket);
}

void tcp_server(void *parv) {
   ESP_LOGE(TAG, "tcp server started.");
   ...
   while (1) {
      ...
      ESP_LOGE(TAG, "Waiting for new client connection");
